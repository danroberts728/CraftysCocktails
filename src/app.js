...
const Recipes = {
  ...
  async load(url=RECIPES_URL){
    ...
    try{
      ...
      console.warn("No recipes loaded:", e);
    }
    return this.map;
  },
  get(name){
...
function setupModal(){
  ...
  // If there's no modal in HTML, inject it (and minimal CSS)
  if (!modal){
    const style = document.createElement("style");
    style.id = "recipe-modal-inline-style";
    style.textContent = `
      .modal{position:fixed;inset:0;display:none;z-index:50;}
      .modal.is-open{display:block;}
      .modal__backdrop{position:absolute;inset:0;background:rgba(0,0,0,.55);backdrop-filter:blur(2px);}
      .modal__panel{position:relative;margin:6vh auto;max-width:960px;background:#0f1421;border:1px solid #2a313f;border-radius:12px;box-shadow:0 10px 40px rgba(0,0,0,.5);padding:16px;color:#f0f3f8;}
      .modal__close{position:absolute;right:10px;top:8px;border:0;background:transparent;color:#9aa3b2;font-size:28px;cursor:pointer;}
      .modal__content{display:grid;grid-template-columns:1.1fr 1.6fr;gap:16px;}
      .modal__media img{width:100%;height:auto;display:block;border-radius:10px;background:#121a2a;}
      .modal__text h2{margin:4px 0 10px;color:#e0b25a;}
      .modal__text h3{margin:14px 0 6px;font-size:1rem;}
      .modal__text ul,.modal__text ol{margin:0 0 10px 18px;}
      .modal__text .notes{color:#9aa3b2;font-style:italic;}
      @media (max-width:820px){.modal__panel{margin:4vh 10px}.modal__content{grid-template-columns:1fr}}
    `;
    ...
    modal.innerHTML = `
      ...
      <div class="modal__panel" role="dialog" aria-modal="true" aria-labelledby="recipe-title">
        <button class="modal__close" type="button" title="Close" data-close>&times;</button>
        <div class="modal__content">
          <div class="modal__media">
            <img id="recipe-photo" alt="" />
          </div>
          <div class="modal__text">
            ...
            <p id="recipe-notes" class="notes"></p>
          </div>
        </div>
      </div>
    `;
...
function openRecipeModal(drinkName){
  ...
  const ol     = document.getElementById("recipe-instructions");
  ...
  if (r){
    ...
    ul.innerHTML = "";
    ol.innerHTML = "";
    notes.textContent = "Recipe coming soon.";
    notes.style.display = "";
  }
...
(async function init(){
  try{
    ...
    const rows = await csv(SHEET_URL);
    const data = rows.map(d=>({
      Round:  +d.Round || 0,
      Match:  +d.Match || 0,
      Drink1: clean(d.Drink1),
      Drink2: clean(d.Drink2),
      Winner: clean(d.Winner),
    }));
